...
from .dataset import generate_batches, ReviewDataset
...
class Learner(object):
    ...
    def train_eval_epoch(self, batch_generator, epoch_index, progress_bar, train_val='train'):
        ...
        for batch_index, batch_dict in enumerate(batch_generator):
            ...
            y_pred = self.classifier(x_in=batch_dict['x_data'].float())
    ...
    def validate(self):
        ...
        for batch_index, batch_dict in enumerate(batch_generator):
            # compute the output
            y_pred = self.classifier(batch_dict['x_data'].float())
    ...
    def show_classifier_top_words(self, n=20, pos_neg='positive'):
        if pos_neg not in ['positive', 'negative']:
            raise ValueError(f'Argument pos_neg must be positive or negative, got {pos_neg}')
        # Sort weights
        fc1_weights = self.classifier.fc1.weight.detach()[0]
        _, indices = torch.sort(fc1_weights, dim=0, descending=True)
        indices = indices.numpy().tolist()

        print(f"Influential words in {pos_neg} Reviews:")
        print("--------------------------------------")
        if pos_neg == 'negative':
            indices.reverse()
        for i in range(n):
            print(self.vectorizer.review_vocab.lookup_index(indices[i]))
